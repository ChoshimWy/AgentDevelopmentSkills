...
Apple/Desktop 安装（包括当前会展开为 Apple 与 Desktop 的 `--platform all`）
时，只要 `cargo` 可用就默认使用 Rust：
...
该路径不存在 Python 回退。源码 checkout 中不属于显式全新
Apple/Desktop/all-ready 选择的请求与其他兼容参数仍要求 Python 3.11+；
PowerShell 也暂时保留该兼容路径。Windows 原生二进制已经进入发布矩阵，但
完整 Windows 安装合同启用前，仍不是 production source-install target。
