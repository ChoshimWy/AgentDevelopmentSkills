...
可执行文件，并且不存在 Python 回退。源码检出安装、交互/兼容参数、
...
- Python 3.11 或更高版本（源码安装和兼容回退仍需要）；
...
该路径不存在 Python 回退。源码 checkout 与其他兼容参数仍要求 Python
3.11+；PowerShell 也暂时保留该兼容路径。Windows 原生二进制已经进入发布
矩阵，但完整 Windows 安装合同启用前，仍不是 production source-install
target。
