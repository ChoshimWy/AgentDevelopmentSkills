...
原生 `LifecycleWorkspace` 现可在持锁期间创建唯一且 POSIX mode 为 `0700` 的 stage/backup 目录对，持有两者的目录 capability，支持 no-follow 递归清理、crash residue 诊断，以及恢复不完整时显式保留 backup。它也可复制并复验符合 Install Plan 局部结构的 Package/Skill 树记录，冻结规范 mode、路径边界、目标原子创建和源 no-follow 遍历；该 API 尚不证明记录属于某个完整已验证 Plan，也不会组装 `AGENTS.md`、Lockfile、外部保留根或交换受管内容。
