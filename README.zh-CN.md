...
bootstrap 在连接终端且不传平台时也会进入同一个 Rust 选择器。兼容参数、
PowerShell bootstrap 升级和旧版本接管仍走独立门禁或 Python 兼容路径。
...
该路径不存在 Python 回退。源码 checkout 中不属于原生交互式或显式全新
Apple/Desktop/all-ready 路由的兼容参数仍要求 Python 3.11+；PowerShell 也
...
从连接终端运行的 POSIX `install.sh` 也会进入 Rust 选择器。`install.ps1` 与
