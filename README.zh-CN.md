...
都会离线构建并执行固定版本的 Rust installer；下载后的签名 POSIX Release
bootstrap 在连接终端且不传平台时也会进入同一个 Rust 选择器。POSIX 下精确的
iOSAgentSkills 旧版软链布局也已默认进入 Rust 接管事务；兼容参数与 PowerShell
bootstrap 升级仍走独立门禁或 Python 兼容路径。
...
仍要求 Python 3.11+；PowerShell 也
暂时保留该兼容路径。Windows 原生二进制已经进入发布矩阵，但完整 Windows
安装合同启用前，仍不是 production source-install target。
...
由同一原生入口接管。`install.ps1` 与
所有不合格请求在本阶段仍走显式 Python 兼容路径。
