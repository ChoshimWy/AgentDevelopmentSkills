...
原生 `LifecycleWorkspace` 现可在持锁期间创建唯一且 POSIX mode 为 `0700` 的 stage/backup 目录对，持有两者的目录 capability，支持 no-follow 递归清理、crash residue 诊断，以及恢复不完整时显式保留 backup。该能力仍只是事务基础设施，尚未复制或交换受管内容。
