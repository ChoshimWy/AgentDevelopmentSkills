...
可执行文件，并且不存在 Python 回退。源码 checkout 的显式或交互式全新安装
都会在 POSIX 上离线构建并执行固定版本的 Rust installer；Windows 源码 checkout
中的显式全新 Desktop 请求也进入同一路径。下载后的
签名 POSIX Release bootstrap 在连接终端且不传平台时也会进入同一个 Rust
选择器。POSIX 下精确的
iOSAgentSkills 旧版软链布局也已默认进入 Rust 接管事务；兼容参数与 PowerShell
托管获取/升级仍走独立门禁或 Python 兼容路径。
...
- Python 3.11 或更高版本（仅显式兼容回退和剩余兼容入口需要）；
...
对于全新目标，只要 `cargo` 可用，不带参数的 `./install.sh` 现在默认使用
Rust 终端选择器：直接回车选择 Apple，输入逗号分隔的平台 ID 可多选，输入
`all` 选择全部 ready 平台，输入 `q` 取消。显式选择全新的 Apple/Desktop
安装（包括当前会展开为 Apple 与 Desktop 的 `--platform all`）也默认使用 Rust：
...
同一 POSIX 源码或签名 Release bootstrap 会识别由 `iOSAgentSkills` checkout
提供的精确 `AGENTS.md` 与 `skills` 双软链布局。Rust 安装器先只读预览，再在
同一生命周期锁事务内保留 `skills/.system` 并移除旧软链；不完整、无关、
跨仓或不安全布局一律 fail-closed。
...
v3 Release 发布后，显式的首次 `--platform apple`、`--platform desktop`
或 `--platform all` 安装或 dry-run 在 macOS 或受支持的 glibc 2.39+ Linux
主机上默认选择 Rust。若先把 Release bootstrap 保存为文件，再从连接终端中
不带 `--platform` 执行，它会进入与源码 checkout 相同的原生行式选择器；上面
的管道命令不具备交互式 stdin，因此显式传入 Apple 默认值。`all` 只展开源码
inventory 中标记为 ready 的平台。
musl 与旧版 glibc 主机继续走 Python 兼容路径。发布门禁
...
该路径不存在 Python 回退。精确 iOSAgentSkills 双软链布局也会进入同一 POSIX
原生路由并在持锁后再次校验。其他不属于原生交互式或显式平台路由的兼容参数
仍要求 Python 3.11+。从源码 checkout 执行的 PowerShell 已把合格的显式全新
Desktop 安装请求路由到隔离、锁定、离线的 Cargo 构建；下载式 PowerShell 获取仍保留
兼容路径。Windows 原生二进制已经进入发布矩阵，但托管 Windows 安装合同启用
前，仍不是 production release-install target。
...
`AGENT_SKILLS_UNINSTALL_ENGINE=rust` 要求必须执行原生路径。源码 checkout
中的兼容 target/platform 请求默认在私有临时 target 内执行 locked、offline
Cargo 构建；如需 Python 3.11+ 兼容路径，可显式选择 `python` engine。原生
构建或执行一旦开始就不会再降级。托管脚本仍会在已安装二进制与内嵌 Release
记录不一致时 fail-closed。
...
兼容命令仍要求调用方显式提供 `agent-session` launcher；合格的托管首次安装则由 v2 bootstrap 把同一份经过校验的原生可执行文件冻结为 launcher，并在事务内完成激活。发布门禁生成的托管 `uninstall.sh` 已切换到 release-matched 原生 guard；源码 checkout 卸载也会为兼容请求默认执行私有、locked、offline Cargo 构建。Release 不匹配、host 不支持、兼容参数或显式选择 Python engine 时，仍走经过校验的 Python 路径。生命周期锁只协调 lifecycle 命令；事务期间调用方仍须保持获批 external scope 静止，并关闭相关可写 handle。
...
公开 `install` 会执行 staging、语义复验、原子发布、发布后复验、失败回滚与清理；`lifecycle-install` 作为只读兼容命令保留。精确 POSIX 旧版布局会进入独立接管事务，临时保留原始软链对象与 `.system`，并在失败时恢复外部预像。Apple 安装还会冻结同一份原生 launcher，并在受保护事务中完成 source activation。安装后的 `agent-session` 继续提供 `create`、`list`、`inspect`、`fingerprint`、`checkpoint` 与 `gate`。
...
源码资格一起纳入合同，并继续把合格托管首次安装的默认引擎设为 Rust；下载后
从连接终端运行的 POSIX `install.sh` 也会进入 Rust 选择器，精确旧版软链布局
由同一原生入口接管。源码 checkout 的 `install.ps1` 已默认用 Rust 处理合格的
显式全新 Desktop 请求；PowerShell Apple/托管获取与所有不合格请求在本阶段仍走
显式 Python 兼容路径。
