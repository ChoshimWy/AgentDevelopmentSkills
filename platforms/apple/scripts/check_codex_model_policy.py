...
CODEX_RUNTIME_ROOT = REPOSITORY_ROOT / "runtime-configs" / "codex" / "assets" / "codex"
SHARED_CONFIG = CODEX_RUNTIME_ROOT / "codex.shared.toml"
AGENT_DIRS = (
    ROOT / "config" / "codex" / "templates" / "agents",
    REPOSITORY_ROOT / "disciplines" / "workflow" / "assets" / "codex" / "agents",
    REPOSITORY_ROOT / "disciplines" / "review" / "assets" / "codex" / "agents",
    REPOSITORY_ROOT / "disciplines" / "design" / "assets" / "codex" / "agents",
)
PROFILE_DIR = CODEX_RUNTIME_ROOT / "profiles"
REQUIRED_PROFILES = {
    "daily",
    "budget",
    "readonly",
...
def validate_static(failures: list[str]) -> list[tuple[str, Path, dict]]:
    ...
    for name, path, data in entries:
        ...
        if not isinstance(model, str) or not model.strip():
            failures.append(f"{name} must declare a model ({path.relative_to(REPOSITORY_ROOT)})")
