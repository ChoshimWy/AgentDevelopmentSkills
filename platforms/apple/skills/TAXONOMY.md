...
本文覆盖 Apple 包自身 Skills，以及 Apple 安装闭包中由共享 Discipline 提供的通用 Skills。Apple 专属能力位于 `platforms/apple/skills/`；`html-docs` 位于 `disciplines/documentation/skills/`，`git-workflow` 与 `gh-pr-flow` 位于 `disciplines/git/skills/`，`workflow-orchestration` 位于 `disciplines/workflow/skills/`，`code-review` 位于 `disciplines/review/skills/`，`ui-ux-design-system` 与 `design-ir-compiler` 位于 `disciplines/design/skills/`，不得在 Apple 包保留重复可安装副本。Apple canonical entry 是 `apple-orchestration`，历史 runtime/vendor 命名不保留兼容副本。
...
- Apple 开发任务默认先进入规范入口 `apple-orchestration`；该 overlay 组合共享 `workflow-orchestration` 的 `lite` / `standard` / `full`、checkpoint、角色和报告合同，再按需路由到统一实施、统一验证、审查、调试、性能与构建模块。
- 修复 / 实现任务即使未手动进入 Plan 模式，也必须在写入前完成 CP0 最小计划；允许先做只读定位，但禁止从代码查找直接进入实现。
- 默认完成标准：定向测试或必要验证通过，且独立 reviewer subAgent 执行共享 `code-review` 并组合 Apple `apple-code-review` extension 后无 `阻塞问题`。
...
- 设计链路先由共享 `ui-ux-design-system` 冻结设计决策，由 `apple-design-source` / 共享 `design_researcher` 提取可追溯 Design Evidence，再由共享 `design-ir-compiler` 生成平台中立 Canonical UI IR，最后由 Apple `apple-design-context-compiler` 生成 iOS bindings 与任务级 Agent Packet；只有合同校验通过后才交给 `ios-feature-implementation`。
- `apple-verification` 统一负责验证前路由、受影响测试选择、定向验证执行、项目环境验证、失败摘要和最终证据裁决。
- `apple-verification` 不再是所有 Apple Xcode 项目改动的强制收尾；只有用户显式要求、发布前自检、高风险、证据不足或需要项目环境证据时才补强执行。
...
- 主入口 `apple-orchestration` 负责自适应编排：所有生产代码和测试代码实施统一切到 `ios-feature-implementation` 的内部模式，验证相关动作统一切到 `apple-verification`。
...
| --- | --- | --- | --- | --- |
| `apple-orchestration` | 默认 Apple canonical entry / Apple overlay | 组合共享 `workflow-orchestration` 后补充 Apple 实施、验证、依赖和工具路由；除 reviewer subAgent 是强制收口角色外，其它 subAgent 使用不做仓库级限制 | 只需要平台无关分型、checkpoint 或报告时直接使用 `workflow-orchestration`；reviewer 不可用时报告 blocked / pending review | `workflow-orchestration`、`ios-feature-implementation`、`apple-verification`、`code-review`、`apple-code-review` |
| `ios-feature-implementation` | 唯一 iOS 代码实施 Skill | service / repository / use case / view model / 导航接线 / SwiftUI / Liquid Glass / UIKit / mixed UI / advanced Swift / refactor / SDK contract / SDK architecture / XCTest / XCUITest / Mock / Stub / Spy / Fake / fixture / Page Object；消费已校验的 iOS Agent Packet | 构建配置、设备自动化、性能 profiling、运行时诊断、官方文档事实检索、纯验证执行、纯静态审查、纯视觉方向探索、设计证据编译 | `apple-verification`、`code-review`、`apple-debugging`、`ios-performance`、`xcode-build`、`apple-docs`、`ui-ux-design-system`、`apple-design-context-compiler` |

## Automation / Build / Validation

| Skill | 角色 | 主触发场景 | 不要触发的场景 | 切换到 |
| --- | --- | --- | --- | --- |
| `ios-automation` | 设备自动化（Simulator + 真机统一入口） | 模拟器生命周期、安装启动、Debug/Test-only Scenario/Fixture、直接页面路由、语义 snapshot、UI smoke、区域截图、UI Summary、无障碍、真机诊断 | Build Settings / 签名策略设计、普通业务实现、默认验证收口 | `xcode-build`、`ios-feature-implementation`、`apple-verification` |
| `xcode-build` | 构建配置与交付链路 | Build Settings、签名、Archive、导出 IPA、CI/CD、scheme、xcconfig、build script、XCFramework | 一次性 build/test 验证、默认收口审查、测试代码编写 | `apple-verification`、`code-review`、`ios-feature-implementation` |
| `apple-verification` | 统一验证入口 | Diff 风险与 evidence requirements、受影响测试、quick/checkpoint/final lane、Verification Session、fingerprint/缓存/in-flight 去重、项目环境 build/test、失败摘要与 Final Evidence Gate | 测试代码编写、构建配置设计、运行时调试、性能 profiling、设备导航自动化 | `ios-feature-implementation`、`code-review`、`xcode-build`、`ios-automation`、`apple-debugging`、`ios-performance` |

## Diagnostics

| Skill | 角色 | 主触发场景 | 不要触发的场景 | 切换到 |
| --- | --- | --- | --- | --- |
| `code-review`（共享） | 跨平台独立静态审查 | review 累计 diff、直接影响面、业务不变量与验证故事；默认由独立 reviewer subAgent 执行 | Apple API、Swift 并发、availability 等平台细节 | `apple-code-review`、`apple-debugging`、`ios-feature-implementation` |
| `apple-code-review` | Apple review extension | 与共享 `code-review` 组合，补充 Swift/Objective-C、并发隔离、availability、文件头、CocoaPods 与 Xcode 证据检查 | 单独替代共享 reviewer 门禁、直接实现或运行测试 | `code-review`、`apple-verification`、`apple-debugging` |
| `apple-debugging` | 运行时排障 | crash、异常、未释放、符号化栈、LLDB 定位 | 纯静态审查、性能分析与 benchmark、构建配置设计 | `code-review`、`ios-performance`、`xcode-build`、`ios-feature-implementation` |
| `ios-performance` | 性能分析与测试 | 掉帧、启动慢、CPU / 内存异常、性能回归基线、`measure(metrics:)`、`xctrace`、Instruments | 通用业务实现、普通单元/UI 测试补齐、泛化 crash 排查 | `ios-feature-implementation`、`apple-verification`、`apple-debugging` |
...
这些技能默认按需触发，不作为 iOS 主链路的第一入口；其中通用技能由共享 Discipline 提供，平台专属技能仍由 Apple 包提供：
- `docs`：`html-docs`（正式 HTML 文档唯一规范入口；承接方案、PRD、评审、报告、任务清单、接口说明与 handoff，并负责统一样式和暗黑模式适配）
- `design`：共享 `ui-ux-design-system`（方向/系统）→ Apple `apple-design-source`（真源 evidence）→ 共享 `design-ir-compiler`（Canonical UI IR）→ Apple `apple-design-context-compiler`（UIKit/SwiftUI bindings 与 Agent Packet）
- `research`：`app-store-changelog`、`app-store-opportunity-research`
- `workflow`：`git-workflow`、`gh-pr-flow`
- `orchestration`：`workflow-orchestration`（共享 checkpoint、角色、fail-fix-report 与 reporting 合同）
- `review`：`code-review`（共享 reviewer 门禁）；Apple 专属检查由 `apple-code-review` extension 提供
