...
- iOS 开发任务默认先进入 Apple 兼容入口 `codex-subagent-orchestration`；该 overlay 组合共享 `workflow-orchestration` 的 `lite` / `standard` / `full`、checkpoint、角色和报告合同，再按需路由到统一实施、统一验证、审查、调试、性能与构建模块。
...
- 设计链路先由共享 `ui-ux-design-system` 冻结设计决策，由 `apple-design-source` / 共享 `design_researcher` 提取可追溯 Design Evidence，再由共享 `design-ir-compiler` 生成平台中立 Canonical UI IR，最后由 Apple `design-context-compiler` 生成 iOS bindings 与任务级 Agent Packet；只有合同校验通过后才交给 `ios-feature-implementation`。
...
| --- | --- | --- | --- | --- |
| `codex-subagent-orchestration` | 默认 iOS 兼容入口 / Apple overlay | 组合共享 `workflow-orchestration` 后补充 Apple 实施、验证、依赖和工具路由；除 reviewer subAgent 是强制收口角色外，其它 subAgent 使用不做仓库级限制 | 只需要平台无关分型、checkpoint 或报告时直接使用 `workflow-orchestration`；reviewer 不可用时报告 blocked / pending review | `workflow-orchestration`、`ios-feature-implementation`、`ios-verification`、`code-review`、`apple-code-review` |
...
| Skill | 角色 | 主触发场景 | 不要触发的场景 | 切换到 |
| --- | --- | --- | --- | --- |
| `code-review`（共享） | 跨平台独立静态审查 | review 累计 diff、直接影响面、业务不变量与验证故事；默认由独立 reviewer subAgent 执行 | Apple API、Swift 并发、availability 等平台细节 | `apple-code-review`、`debugging`、`ios-feature-implementation` |
| `apple-code-review` | Apple review extension | 与共享 `code-review` 组合，补充 Swift/Objective-C、并发隔离、availability、文件头、CocoaPods 与 Xcode 证据检查 | 单独替代共享 reviewer 门禁、直接实现或运行测试 | `code-review`、`ios-verification`、`debugging` |
...
- `design`：共享 `ui-ux-design-system`（方向/系统）→ Apple `apple-design-source`（真源 evidence）→ 共享 `design-ir-compiler`（Canonical UI IR）→ Apple `design-context-compiler`（UIKit/SwiftUI bindings 与 Agent Packet）
