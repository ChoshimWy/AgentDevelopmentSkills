---
name: apple-debugging
description: Apple 平台运行时调试与问题排查 Skill。用于 iOS、macOS、watchOS、tvOS、visionOS 的 crash、异常、运行时错误、对象未释放、内存泄漏、Watchdog、卡死与行为异常；不要把编译验证、静态代码审查、性能 profiling、benchmark 或构建配置误判到本 Skill。
---
...
Diagnose Apple-platform runtime failures using symptoms, logs, stack traces, reproduction steps, and runtime evidence, then provide root-cause analysis, focused fixes, and verification guidance.
...
- The request is final compile/build verification; use `apple-verification` / `apple-verification`.
...
- The request is compact attribution of a build/test failure artifact rather than runtime diagnosis; use `apple-verification`.
...
- `apple-verification` / `apple-verification` are optional strengthening steps only when explicitly requested or risk requires it.
...
- For build/test failure logs encountered during debugging, use `apple-verification`.
...
Escalate to `apple-verification` only when:

- The user explicitly asks for project-environment verification.
- `apple-verification` determines the debugging fix needs full build evidence.
...
- Build verification: use `apple-verification`.
- Build/test log attribution: use `apple-verification`.
- Test writing: use `ios-feature-implementation(test-implementation)`; targeted validation: use `apple-verification`.
