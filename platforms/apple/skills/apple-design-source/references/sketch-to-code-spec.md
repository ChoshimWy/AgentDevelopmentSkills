...
用于把 Sketch 源文件、SketchMCP、蓝湖 / PNG 补充素材转换为可追溯的 `Design Evidence / Design-to-Code source packet`，再交给 `apple-design-context-compiler` 生成 Canonical UI IR、iOS bindings 与任务级 Agent Packet；合同校验通过后才交给 `ios-feature-implementation` 的 SwiftUI / UIKit / mixed-ui 模式实现。
...
- iOS 实现还必须经过 `apple-design-context-compiler` 生成并校验任务级 Agent Packet；不得从本 source packet 或完整 Sketch JSON 直接跳到产品代码。
