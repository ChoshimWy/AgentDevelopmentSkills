...
不直接编写 UIKit / SwiftUI 产品代码，不替代共享 `ui-ux-design-system` / `design-ir-compiler`，不替代 `ios-automation` 的截图执行，也不替代 `ios-verification` / `code-review` 的最终门禁。现有 `normalize` 仅作为迁移期兼容 mode，必须语义等价地委托共享 IR base，不得演化出第二套通用合同。
