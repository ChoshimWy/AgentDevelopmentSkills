...
- 涉及代码改动时，`apple-verification` 默认只执行最窄定向单测；真机 / 模拟器验证不属于默认执行面。
- `apple-verification` 仅作为按需补强验证，由主 Agent 在用户显式要求、发布前自检或高风险时执行。
...
## 基线复用
- 若最后一次代码变更之后已经成功执行最窄定向单测，且未命中工程/依赖/签名/资源/设备能力高风险条件，`apple-verification` 可直接接受该证据，不因缺少真机 / 模拟器验证而默认升级。
- 如果同一任务已经先跑过定向测试或其它 build/test，`apple-verification` 默认复用同一套 workspace / scheme / destination 基线。
