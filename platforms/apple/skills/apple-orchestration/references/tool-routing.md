...
- 用户明确指定 `.sketch` 源文件、画板或 Sketch 设计真源：启动只读 `design_researcher`，由其专属 `sketchMCP`（`http://localhost:31126/mcp`）读取图层与样式事实并输出 Design Evidence / Design-to-Code source packet；再由 `apple-design-context-compiler` 生成 Canonical UI IR、iOS bindings 与任务级 Agent Packet。MCP 未启动或源文件不可读时报告 blocked，不以截图或既有 UI 猜测替代。
...
## 构建、测试与设备
- 日常最窄 build/test 使用 `apple-verification(quick-verify)`，由 `codex_verify`、Verification Session 和 shared build-queue 负责发现、fingerprint、缓存与 in-flight 去重。不要使用 Xcode MCP、第三方 Xcode GUI MCP、AppleScript 或 Accessibility 控制 Xcode。
- `tester` 做定向验证、失败归因、日志查看时，优先复用 `apple-verification`；设备/截图/导航证据才切 `ios-automation`。
