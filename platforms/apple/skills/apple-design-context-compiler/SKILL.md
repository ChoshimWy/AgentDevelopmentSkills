---
name: apple-design-context-compiler
description: Apple Design Binding Compiler extension。消费共享 design-ir-compiler 生成的平台中立 Canonical UI IR，解析 UIKit / SwiftUI Component Registry 与 bindings，并生成 iOS Agent Packet、实施清单和 benchmark 证据；不再拥有通用设计系统或 Canonical IR 基础合同。
---

# Apple Design Binding Compiler

## Purpose

Extend the shared Canonical UI IR with validated UIKit/SwiftUI bindings and a task-scoped iOS implementation packet without passing raw design-tool JSON to the implementation agent.
...
不直接编写 UIKit / SwiftUI 产品代码，不替代共享 `ui-ux-design-system` / `design-ir-compiler`，不替代 `ios-automation` 的截图执行，也不替代 `apple-verification` / `code-review` 的最终门禁。现有 `normalize` 仅作为迁移期兼容 mode，必须语义等价地委托共享 IR base，不得演化出第二套通用合同。
...
- 任务是 Xcode build/test 或证据裁决；使用 `apple-verification`。
...
| `normalize` | 迁移期兼容 alias；委托共享 `design-ir-compiler(normalize)` | `ui-ir.json` |
...
- Escalate build/test evidence to `apple-verification` and static final review to independent `code-review`.
...
- 共享 `ui-ux-design-system` 提供设计决策，`apple-design-source` 提供 Apple 真源 evidence。
- 共享 `design-ir-compiler` 生成 Canonical UI IR；本 Skill 只编译 iOS bindings 和 Agent Packet。
- `ios-feature-implementation` 是唯一 UIKit / SwiftUI / mixed-ui 产品代码实施入口。
- `ios-automation` 采集截图、accessibility tree 与 UI smoke 证据。
- `apple-verification` 与独立 `code-review` 负责实现链路收口。
